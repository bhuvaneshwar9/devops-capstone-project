...
class TestAccountService(TestCase):
    ...
    def test_update_account_not_found(self):
        ...
        resp = self.client.put(f"{BASE_URL}/0", json={"name": "not found"})
        self.assertEqual(resp.status_code, status.HTTP_404_NOT_FOUND)
    ...
    def test_create_account_no_content_type(self):
        ...
        account = AccountFactory()
        resp = self.client.post(BASE_URL, data=account.serialize())
    ...
    def test_account_deserialize_missing_data(self):
        """It should not deserialize an Account with missing data"""
        data = {"address": "missing data"}
        account = Account()
        self.assertRaises(
            DataValidationError, account.deserialize, data
        )

    def test_account_deserialize_bad_data(self):
        """It should not deserialize an Account with bad data"""
        data = {"id": "1", "name": "John", "email": "john@example.com", "address": "123 Main St", "phone_number": "123-456-7890", "date_joined": "not a date"}
        account = Account()
        self.assertRaises(
            ValueError, account.deserialize, data
        )